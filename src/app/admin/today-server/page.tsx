...
export default async function TodayServerPage() {
  ...
  try {
    ...
    const { data: arrivals, error: arrivalsError } = await adminClient
      .from('bookings')
      .select('id, tenant_id, reference, customer_name, customer_email, customer_phone, plate, car_make, car_model, car_color, start_at, end_at, status, money_received, money_charged, source, flight_number, notes, stripe_payment_intent_id, payment_status, checked_in_at, checked_out_at')
