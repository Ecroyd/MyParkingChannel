// app/api/payments/checkout/route.ts
import { NextResponse } from 'next/server';
import { stripe, useConnected, ROOT_URL } from '@/lib/stripe';
import { getServerSupabase, getAuthedUserTenantId, getTenantStripeAccountId } from '@/lib/supabase-server';
import { getQuoteCents } from '@/lib/pricing';

/**
 * POST body:
 * {
 *   booking_id?: string,                // if known (existing booking)
 *   start_at?: string, end_at?: string, // ISO strings for new quote
 *   customer_name?: string,
 *   reference?: string,                 // optional, displayed in product name
 *   application_fee_cents?: number      // optional platform fee; default 0
 * }
 */
export async function POST(req: Request) {
  const body = await req.json().catch(() => ({}));
  const {
    booking_id,
    start_at,
    end_at,
    customer_name,
    reference,
    application_fee_cents = 0,
  } = body;

  const tenantId = await getAuthedUserTenantId();
  const { accountId } = await getTenantStripeAccountId(tenantId);
  if (!accountId) return NextResponse.json({ error: 'Stripe not connected' }, { status: 400 });

  const supabase = getServerSupabase();

  // Determine price from either an existing booking or new quote
  let amount_cents: number;
  let currency: string;

  if (booking_id) {
    const { data: b, error } = await supabase
      .from('bookings')
      .select('id, tenant_id, start_at, end_at, customer_name, reference')
      .eq('id', booking_id)
      .eq('tenant_id', tenantId)
      .maybeSingle();
    if (error) throw error;
    if (!b) return NextResponse.json({ error: 'Booking not found' }, { status: 404 });

    const q = await getQuoteCents(tenantId, b.start_at, b.end_at);
    amount_cents = q.amount_cents;
    currency = q.currency;
  } else {
    if (!start_at || !end_at) {
      return NextResponse.json({ error: 'Provide (start_at, end_at) or booking_id' }, { status: 400 });
    }
    const q = await getQuoteCents(tenantId, start_at, end_at);
    amount_cents = q.amount_cents;
    currency = q.currency;
  }

  if (amount_cents <= 0) return NextResponse.json({ error: 'Invalid amount' }, { status: 400 });

  const productName = `Parking (${reference ?? 'new'})`;
  const descName = customer_name ? `Customer: ${customer_name}` : undefined;

  const session = await stripe.checkout.sessions.create(
    {
      mode: 'payment',
      line_items: [{
        price_data: {
          currency,
          unit_amount: amount_cents,
          product_data: { name: productName, description: descName },
        },
        quantity: 1,
      }],
      payment_intent_data: {
        application_fee_amount: Number(application_fee_cents) || 0,
        metadata: {
          tenant_id: tenantId,
          booking_id: booking_id ?? '',
        },
      },
      success_url: `${ROOT_URL}/admin/bookings?paid=1`,
      cancel_url: `${ROOT_URL}/admin/bookings?cancelled=1`,
    },
    useConnected(accountId),
  );

  return NextResponse.json({ url: session.url });
}
